...
public final class RevivalNetwork {
 ...
 public static BodyView view(ServerPlayer viewer,ServerPlayer subject,BodySnapshot snapshot,Region selected,int requestedPage){
  ...
  var supplies=new ArrayList<Integer>();for(var type:MaimType.values()){int count=0;for(int slot=0;slot<viewer.getInventory().getContainerSize();slot++){var stack=viewer.getInventory().getItem(slot);if(stack.is(InjuryItems.treatmentTag(type)))count+=stack.getCount();}supplies.add(count);}
  return new BodyView(subject.getUUID(),subject.getGameProfile().getName(),snapshot.health(),snapshot.maxHealth(),snapshot.atDoor(),(int)snapshot.healingLockTicks(),snapshot.traumaCount(),snapshot.tuning().traumaLifetimeTicks(),snapshot.deathProbability(),snapshot.functionalMultiplier(),RevivalManager.snapshot(viewer).treatmentSeconds(),rows,history,page,pages,supplies);
 }
