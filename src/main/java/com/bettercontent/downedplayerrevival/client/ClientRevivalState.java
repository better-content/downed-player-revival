...
public final class ClientRevivalState {
 ...
 private static BodyView recap;private static DamageLedger.Summary recapDamage=DamageLedger.Summary.empty();private static boolean recapWasDead;private static int captureDelay=-1;private static String captureLabel;
 ...
 public static void accept(StateSyncPacket packet){
  ...
  if(packet.mode()==2){recap=body;recapDamage=packet.damage();recapWasDead=false;return;}
 ...
 public static void control(UiControlPacket packet){var mc=Minecraft.getInstance();switch(packet.operation()){
  ...
  case "failed-proc"->{if(InjuryClientConfig.SOUND.get()&&mc.player!=null)mc.player.playSound(SoundEvents.ANVIL_LAND,.8f,.55f);}
 ...
 public static void tick(Minecraft mc){if(mc.player==null){STATES.clear();return;}if(recap!=null&&!mc.player.isAlive())recapWasDead=true;if(recapWasDead&&mc.player.isAlive()){recap=null;recapDamage=DamageLedger.Summary.empty();recapWasDead=false;}if(captureDelay>=0&&--captureDelay==0){Screenshot.grab(mc.gameDirectory,captureLabel+".png",mc.getMainRenderTarget(),message->System.out.println("INJURY_GUI_CAPTURE "+message.getString()));captureDelay=-1;}}
 public static void clear(){STATES.clear();recap=null;recapDamage=DamageLedger.Summary.empty();recapWasDead=false;captureDelay=-1;captureLabel=null;RevivalHud.reset();}
}
