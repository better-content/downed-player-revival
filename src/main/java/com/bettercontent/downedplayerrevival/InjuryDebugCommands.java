...
public final class InjuryDebugCommands {
 ...
 public static void register(CommandDispatcher<CommandSourceStack> dispatcher){
  ...
  dispatcher.register(literal("downedplayerrevival").requires(s->s.hasPermission(2)).then(literal("debug").then(gui).then(scenario).then(capture).then(treatment).then(pressure).then(preferences)));
 }
}
