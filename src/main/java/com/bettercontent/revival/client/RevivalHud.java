...
public final class RevivalHud {
    ...
    private static void renderDowned(GuiGraphics graphics, Minecraft minecraft, StateSyncPacket state) {
        ...
        if (state.downedTicks() < unlock) {
            graphics.drawCenteredString(minecraft.font,
                    Component.translatable("revival.hud.give_up_locked", formatTicks(unlock - state.downedTicks())), center, y + 25, MUTED);
            ...
            graphics.drawCenteredString(minecraft.font, Component.translatable("revival.hud.giving_up"), center, y + 28, WHITE);
        } else {
            graphics.drawCenteredString(minecraft.font, Component.translatable("revival.hud.give_up"), center, y + 25, MUTED);
        }
    }
    ...
    private static void renderAid(GuiGraphics graphics, Minecraft minecraft) {
        ...
        graphics.drawCenteredString(minecraft.font,
                Component.translatable("revival.hud.reviving", target.getDisplayName()), center, y + 4, WHITE);
        if (state.helperCount() > 1) {
            graphics.drawCenteredString(minecraft.font,
                    Component.translatable("revival.hud.helpers", state.helperCount()), center, y + 16, MUTED);
        }
    }
