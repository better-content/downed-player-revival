...
public final class InjuryVisualReview {
 ...
 @SubscribeEvent public static void tick(TickEvent.ClientTickEvent event) throws Exception {
  ...
  if(!ready){if(++ticks<30)return;ready=true;ticks=0;prepare();command("gamerule doDaylightCycle false");command("gamerule naturalRegeneration false");command("gamerule doMobSpawning false");command("gamerule announceAdvancements false");command("time set noon");command("gamerule sendCommandFeedback false");if(MULTIPLAYER)command("execute at @a run tp InjuryHelper ~2 ~ ~");if(!MANUAL)next();else { GLFW.glfwSetWindowSize(mc.getWindow().getWindow(),1280,960);mc.options.guiScale().set(4);mc.resizeDisplay();System.out.println("INJURY_REVIEW_MANUAL_READY"); }return;}
  Path queue=mc.gameDirectory.toPath().resolve("review.commands");if(Files.isRegularFile(queue)){var commands=Files.readAllLines(queue);Files.delete(queue);for(String line:commands){if(line.equals("stop")){mc.stop();return;}if(!line.isBlank())command(line);}}
  if(MANUAL)return;
 ...
 private static void next(){var mc=Minecraft.getInstance();if(++index>=FRAMES.size()){System.out.println("INJURY_REVIEW_COMPLETE frames="+FRAMES.size());if(MULTIPLAYER)try{Files.writeString(mc.gameDirectory.toPath().resolve("../injury-helper/review.stop"),"stop");}catch(java.io.IOException e){throw new RuntimeException(e);}mc.stop();return;}var frame=FRAMES.get(index);GLFW.glfwSetWindowSize(mc.getWindow().getWindow(),frame.width,frame.height);mc.options.guiScale().set(frame.scale);mc.resizeDisplay();for(String c:frame.commands)command(c);ticks=0;capturing=false;}
